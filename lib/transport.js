...
module.exports.createServer = function (options) {
	...
	console.log("local account fingerprint:", options.account.fingerprint());
...
function secureStream(options) {
	...
	var chan0_in = peer.createReadStream(0);
	var chan0_out = peer.createWriteStream(0);
	var chan1_in = peer.createReadStream(1);
	var chan1_out = peer.createWriteStream(1);

	var remote_endpoint = peer.address().address() + peer.address().port; //sha1(address+date.now)?
	var remote = account.contact(remote_endpoint);
	...
	session.on("gone_secure", function () {
		...
		remoteFingerprint = session.theirFingerprint();
	...
	session.on("received_symkey", function (use, usedata, extraKeyReceiving) {
		if (!secured) return;
		//just testing - review http://tools.ietf.org/html/rfc3686
